...
# Note numbers are from their midi form:
# https://newt.phys.unsw.edu.au/jw/notes.html
# At the moment, recognition below B1 breaks if max is above 48
NOTE_MIN = 28       # E1
NOTE_MAX = 60       # C3
...
ampCut = 0
parser.add_argument('-c', dest='ampCut', default=0, action='store', type=int)
...
# Print initial text
print 'cutoff amplitude: {:d}'.format(ampCut)
...
while stream.is_active():
    ...
    # get the amplitude... I hope
    blockLinearRms= np.sqrt(np.mean(buf**2)) # Linear value between 0 -> 1
    ...
    if num_frames >= FRAMES_PER_FFT:
        if (blockLogRms >= ampCut) or (ampCut == 0):
            print 'freq: {:7.2f} Hz     note: {:>3s} {:+.2f} amp: {:.2f}'.format(
                freq, note_name(n0), n-n0, blockLogRms)